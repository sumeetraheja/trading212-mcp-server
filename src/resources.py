...
#
# Note: the root segment is `accounts` (plural) here, distinct from the legacy
# singular `account` root above. This avoids URI routing ambiguity where the
# `{account}` wildcard would otherwise match legacy static segments like
# `info` or `cash`. Do not "normalise" these to `account/{account}/...`.
@mcp.resource("trading212://accounts/{account}/info")
...
@mcp.resource("trading212://accounts/{account}/cash")
...
@mcp.resource("trading212://accounts/{account}/portfolio")
...
@mcp.resource("trading212://accounts/{account}/portfolio/{ticker}")
def get_account_position_by_ticker_for(account: str, ticker: str) -> Position:
    ...
    return registry.get_client(account).get_account_position_by_ticker(ticker=ticker)


@mcp.resource("trading212://accounts/{account}/orders")
...
@mcp.resource("trading212://accounts/{account}/orders/{order_id}")
def get_order_by_id_for(account: str, order_id: int) -> Order:
    """Fetch a specific order by ID for a specific account."""
    return registry.get_client(account).get_order_by_id(order_id=order_id)


@mcp.resource("trading212://accounts/{account}/pies")
...
@mcp.resource("trading212://accounts/{account}/pies/{pie_id}")
def get_pie_by_id_for(account: str, pie_id: int) -> AccountBucketResultResponse:
    ...
    return registry.get_client(account).get_pie_by_id(pie_id=pie_id)


@mcp.resource("trading212://accounts/{account}/history/exports")
